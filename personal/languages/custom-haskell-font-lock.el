;;; custom-haskell-font-lock.el --- Font locking module for custom-haskell Mode
...
;; Keywords: faces files custom-haskell
...
;; Purpose:
;;
;; To support fontification of standard custom-haskell keywords, symbols,
;; functions, etc.  Supports full custom-haskell 1.4 as well as LaTeX- and
;; Bird-style literate scripts.
;;
;; Installation:
;;
;; To turn font locking on for all custom-haskell buffers under the custom-haskell
;; mode of Moss&Thorn, add this to .emacs:
;;
;;    (add-hook 'haskell-mode-hook 'turn-on-custom-haskell-font-lock)
;;
;; Otherwise, call `turn-on-custom-haskell-font-lock'.
;;
;;
;; Customisation:
;;
;; The colours and level of font locking may be customised.  See the
;; documentation on `turn-on-custom-haskell-font-lock' for more details.
;;
...
;; `(turn-(on/off)-)custom-haskell-font-lock' or `haskell-fl-'.
...
;;
;; Version 1.0:
;;   Brought over from custom-haskell mode v1.1.
...
(defcustom custom-haskell-font-lock-symbols nil
...
(defconst custom-haskell-font-lock-symbols-alist
  (append
   ;; Prefer single-width Unicode font for lambda.
   (and (fboundp 'decode-char)
        (memq custom-haskell-font-lock-symbols '(t unicode))
        (list (cons "\\" (decode-char 'ucs 955))))
   ;; The symbols can come from a JIS0208 font.
   (and (fboundp 'make-char) (fboundp 'charsetp) (charsetp 'japanese-jisx0208)
        (memq custom-haskell-font-lock-symbols '(t japanese-jisx0208))
   ...
   (and (fboundp 'decode-char)
        ...
        (list (cons "not" (decode-char 'ucs 172))
              (cons "<<<" (decode-char 'ucs #X22D8))
              (cons ">>>" (decode-char 'ucs #X22D9))
              (cons "+++" (decode-char 'ucs #X2A01))
              (cons "<*>" (decode-char 'ucs #X22C6))
              (cons "<|>" (decode-char 'ucs #XFF5C))
              (cons "<$>" (decode-char 'ucs #X22A1))
              (cons "<+>" (decode-char 'ucs #X229E))
              (cons "***" (decode-char 'ucs #X225B))
              (cons "=<<" (decode-char 'ucs #X2266))
              (cons ">>=" (decode-char 'ucs #X2267))
              (cons "|>"  (decode-char 'ucs #X22AB))
              (cons "<|"  (decode-char 'ucs #X2AE5))
              ...
              (cons "<>"  (decode-char 'ucs #X27D0))
              (cons "<<"  (decode-char 'ucs #X226A))
              (cons ">>"  (decode-char 'ucs #X226B))
              (cons "++"  (decode-char 'ucs #X2295))
              ...
              (cons ">-" (decode-char 'ucs 8611)) ;; Paterson's arrow syntax
              ...
              (list "." (decode-char 'ucs 8728) ; (decode-char 'ucs 9675)
                    ...
                    ;; forall <foo> . <bar>.
                    'custom-haskell-font-lock-dot-is-not-composition)
              ;; (cons "forall" (decode-char 'ucs 8704))
              )))
  "Alist mapping custom-haskell symbols to chars.
Each element has the form (STRING . CHAR) or (STRING CHAR PREDICATE).
STRING is the custom-haskell symbol.
...
(defun custom-haskell-font-lock-dot-is-not-composition (start)
...
(defvar custom-haskell-keyword-face 'font-lock-keyword-face)
(defvar custom-haskell-constructor-face 'font-lock-type-face)
...
(defvar custom-haskell-definition-face 'font-lock-function-name-face)
;; This is probably just wrong, but it used to use
;; `font-lock-function-name-face' with a result that was not consistent with
;; other major modes, so I just exchanged with `haskell-definition-face'.
(defvar custom-haskell-operator-face 'font-lock-variable-name-face)
(defvar custom-haskell-pragma-face 'font-lock-comment-face)
(defvar custom-haskell-default-face nil)
(defvar custom-haskell-literate-comment-face 'font-lock-doc-face
  "Face with which to fontify literate comments.
Set to `default' to avoid fontification of them.")

(defconst custom-haskell-emacs21-features (string-match "[[:alpha:]]" "x")
  "Non-nil if we have regexp char classes.
Assume this means we have other useful features from Emacs 21.")

(defun custom-haskell-font-lock-compose-symbol (alist)
...
(defun custom-haskell-font-lock-symbols-keywords ()
  (when (fboundp 'compose-region)
    (let ((alist nil))
      (dolist (x custom-haskell-font-lock-symbols-alist)
      ...
      (when alist
        `((,(regexp-opt (mapcar 'car alist) t)
           (0 (custom-haskell-font-lock-compose-symbol ',alist)
              ;; In Emacs-21, if the `override' field is nil, the face
              ;; expressions is only evaluated if the text has currently
              ;; no face.  So force evaluation by using `keep'.
              keep)))))))

(defun custom-haskell-font-lock-find-pragma (end)
  (catch 'custom-haskell-font-lock-find-pragma
      ...
      (let* ((begin (match-beginning 0))
        ...
        (unless (or (nth 3 ppss)
          ...
          (let ((end (scan-lists begin 1 0)))
            ...
            (goto-char end)
            (throw 'custom-haskell-font-lock-find-pragma t)))))
    ;; Found no pragma.
    nil))

;; The font lock regular expressions.
(defun custom-haskell-font-lock-keywords-create (literate)
  "Create fontification definitions for custom-haskell scripts.
Returns keywords suitable for `font-lock-keywords'."
  (let* (;; Bird-style literate scripts start a line of code with
         ...
         ;; Most names are borrowed from the lexical syntax of the custom-haskell
         ;; report.
    ...
    (setq keywords
          `(;; NOTICE the ordering below is significant
            ...
            ,@(unless custom-haskell-emacs21-features ;Supports nested comments?
                ;; Expensive.
            ...
            ,@(custom-haskell-font-lock-symbols-keywords)
            ...
            (,sym 0 (if (eq (char-after (match-beginning 0)) ?:)
                        custom-haskell-constructor-face
                      custom-haskell-operator-face))

            (custom-haskell-font-lock-find-pragma 0 custom-haskell-pragma-face t)))
    (unless (boundp 'font-lock-syntactic-keywords)
      (case literate
        (bird
         (setq keywords
               `(("^[^>\n].*$" 0 custom-haskell-comment-face t)
                 ,@keywords
                 ("^>" 0 custom-haskell-default-face t))))
        ((latex tex)
         (setq keywords
...
(defvar custom-haskell-fl-latex-cache-pos nil
  "Position of cache point used by `haskell-fl-latex-cache-in-comment'.
Should be at the start of a line.")

(defvar custom-haskell-fl-latex-cache-in-comment nil
...
(defun custom-haskell-fl-latex-comments (end)
    ...
    (if (= start end)
      ...
      (if (not (eq start custom-haskell-fl-latex-cache-pos))
          ...
          (progn
            ...
            ;; Restore position.
            (goto-char start)))
      (if custom-haskell-fl-latex-cache-in-comment
          (progn
...
(defconst custom-haskell-basic-syntactic-keywords
...
(defconst custom-haskell-bird-syntactic-keywords
  (cons '("^[^\n>]"  (0 "<"))
        custom-haskell-basic-syntactic-keywords))

(defconst custom-haskell-latex-syntactic-keywords
  (append
   '(("^\\\\begin{code}\\(\n\\)" 1 "!")
...
(defcustom custom-haskell-font-lock-haddock (boundp 'font-lock-doc-face)
  "If non-nil try to highlight Haddock comments specially."
  :type 'boolean
  :group 'haskell)

(defvar custom-haskell-font-lock-seen-haddock nil)
(make-variable-buffer-local 'custom-haskell-font-lock-seen-haddock)

(defun custom-haskell-syntactic-face-function (state)
  "`font-lock-syntactic-face-function' for custom-haskell."
  (cond
   ...
   ((or (eq 'syntax-table (nth 7 state))
    ...
    custom-haskell-literate-comment-face)
   ...
   ((and custom-haskell-font-lock-haddock
         (save-excursion
           (goto-char (nth 8 state))
           (or (looking-at "\\(-- \\|{-\\)[ \\t]*[|^]")
               (and custom-haskell-font-lock-seen-haddock
                    (looking-at "-- ")
                    (let ((doc nil)
                          pos)
                      (while (and (not doc)
                      ...
                      doc)))))
    (set (make-local-variable 'custom-haskell-font-lock-seen-haddock) t)
    font-lock-doc-face)
   (t font-lock-comment-face)))

(defconst custom-haskell-font-lock-keywords
  (custom-haskell-font-lock-keywords-create nil)
  "Font lock definitions for non-literate custom-haskell.")

(defconst custom-haskell-font-lock-bird-literate-keywords
  (custom-haskell-font-lock-keywords-create 'bird)
  "Font lock definitions for Bird-style literate custom-haskell.")

(defconst custom-haskell-font-lock-latex-literate-keywords
  (custom-haskell-font-lock-keywords-create 'latex)
  "Font lock definitions for LaTeX-style literate custom-haskell.")

;;;###autoload
(defun custom-haskell-font-lock-choose-keywords ()
  (let ((literate (if (boundp 'haskell-literate) haskell-literate)))
    (case literate
      (bird custom-haskell-font-lock-bird-literate-keywords)
      ((latex tex) custom-haskell-font-lock-latex-literate-keywords)
      (t custom-haskell-font-lock-keywords))))

(defun custom-haskell-font-lock-choose-syntactic-keywords ()
  (let ((literate (if (boundp 'haskell-literate) haskell-literate)))
    (case literate
      (bird custom-haskell-bird-syntactic-keywords)
      ((latex tex) custom-haskell-latex-syntactic-keywords)
      (t custom-haskell-basic-syntactic-keywords))))

(defun custom-haskell-font-lock-defaults-create ()
  "Locally set `font-lock-defaults' for custom-haskell."
  (set (make-local-variable 'font-lock-defaults)
       '(custom-haskell-font-lock-choose-keywords
         nil nil ((?\' . "w") (?_  . "w")) nil
         (font-lock-syntactic-keywords
          . custom-haskell-font-lock-choose-syntactic-keywords)
         (font-lock-syntactic-face-function
          . custom-haskell-syntactic-face-function)
         ;; Get help from font-lock-syntactic-keywords.
         (parse-sexp-lookup-properties . t))))

;; The main functions.
(defun turn-on-custom-haskell-font-lock ()
  "Turns on font locking in current buffer for custom-haskell 1.4 scripts.
...
.emacs:
  ...
  (add-hook 'custom-haskell-font-lock-hook
      (lambda ()
...
To turn font locking on for all custom-haskell buffers, add this to .emacs:

  (add-hook 'haskell-mode-hook 'turn-on-custom-haskell-font-lock)

To turn font locking on for the current buffer, call
`turn-on-custom-haskell-font-lock'.  To turn font locking off in the current
buffer, call `turn-off-custom-haskell-font-lock'.

Bird-style literate custom-haskell scripts are supported: If the value of
`haskell-literate-bird-style' (automatically set by the custom-haskell mode
of Moss&Thorn) is non-nil, a Bird-style literate script is assumed.

Invokes `custom-haskell-font-lock-hook' if not nil."
  (custom-haskell-font-lock-defaults-create)
  (run-hooks 'custom-haskell-font-lock-hook)
  (turn-on-font-lock))

(defun turn-off-custom-haskell-font-lock ()
  "Turns off font locking in current buffer."
  (font-lock-mode -1))

;; Provide ourselves:

(provide 'custom-haskell-font-lock)
;;; custom-haskell-font-lock.el ends here
